block_time = range/(v_cruise*3.6) + 0.25; % [h]
% flight cost
...
BEP = 2.451e-14*(T_curr*kg2lb/2)^3-2.846e-9*...
    (T_curr*kg2lb/2)^2+1.838e-4*(T_curr*kg2lb/2)+0.6321;
...
E_labour = 0.21 * man_hour_cost * C1 * C3 * (1+T_curr* 1e-3)^0.4;
E_material = 2.56 * (1+T_curr* 1e-3)^0.8 * C1*(C2+C3);

maintenance_cost = A_labour + A_material + N_prop * (E_labour+E_material)...
    * (flight_time + 1.3)/block_time; % [$/h]

total_cost = flight_cost + maintenance_cost; % [$/h]
Vb = range / block_time; % [km/h] velocità commerciale
DOC = total_cost / (passeggeri * Vb); % direct operating cost

CO2_emissions = 3.16 * W_block_fuel;

% Payload-Range Efficiency
PREE = (W_payload * range) / (FED * W_block_fuel);