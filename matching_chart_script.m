...
C_L_max_flapped = C_L_max_3D_clean + delta_C_L_flap;
...
% first segment
gamma1 = atan(0/100);
thrust_ratio1 = 2*(0.5*rho_SL*(1.2*Vstall*kmph2mps)^2/(W_S_des*g)*((Cd0+C_D_flap+C_D_LG) + ...
    k_polare*(2*(W_S_des*g)*cos(gamma1)/(rho_SL*(1.2*Vstall*kmph2mps)^2))^2) + sin(gamma1));
% second segment
gamma2 = atan(2.4/100);
thrust_ratio2 = 2*(0.5*rho_SL*(1.2*Vstall*kmph2mps)^2/(W_S_des*g)*((Cd0+C_D_flap) + ...
    k_polare*(2*(W_S_des*g)*cos(gamma2)/(rho_SL*(1.2*Vstall*kmph2mps)^2))^2) + sin(gamma2));
% third segment
gamma3 = atan(1.2/100);
thrust_ratio3 = 2*(0.5*rho_SL*(1.25*Vstall*kmph2mps)^2/(W_S_des*g)*((Cd0) + ...
    k_polare*(2*(W_S_des*g)*cos(gamma3)/(rho_SL*(1.25*Vstall*kmph2mps)^2))^2) + sin(gamma3));
...
thrust_ratio_ldg_climb = 2*(0.5*rho_SL*(vLdgClimb*kmph2mps)^2/(W_S_des*g)*((Cd0+C_D_flap+C_D_LG) + ...
    k_polare*(2*(W_S_des*g)*cos(gammaLdgClimb)/(rho_SL*(vLdgClimb*kmph2mps)^2))^2) + sin(gammaLdgClimb));

% Approach climb
vAppClimb = 1.41*Vstall;
gammaAppClimb = atan(2.1/100);
C_D_flap = 0.9 * (1/4.1935)^1.38 * Sflap/S_ref * sind(20)^2;
thrust_ratio_app_climb = 2*(0.5*rho_SL*(vAppClimb*kmph2mps)^2/(W_S_des*g)*((Cd0+C_D_flap) + ...
    k_polare*(2*(W_S_des*g)*cos(gammaAppClimb)/(rho_SL*(vAppClimb*kmph2mps)^2))^2) + sin(gammaAppClimb));
