...
% first segment
gamma1 = atan(0/100);
thrust_ratio1 = 2*(0.5*rho_SL*(1.2*Vstall*kmph2mps)^2/(W_S_des*g)*((CD0_clean+C_D_flap+C_D_LG) + k_polare*(2*(W_S_des*g)*cos(gamma1)/(rho_SL*(1.2*Vstall*kmph2mps)^2))^2) + sin(gamma1));
% second segment
gamma2 = atan(2.4/100);
...
% third segment
gamma3 = atan(1.2/100);
thrust_ratio3 = 2*(0.5*rho_SL*(1.25*Vstall*kmph2mps)^2/(W_S_des*g)*((CD0_clean) + k_polare*(2*(W_S_des*g)*cos(gamma3)/(rho_SL*(1.25*Vstall*kmph2mps)^2))^2) + sin(gamma3));

thrust_ratio_climb = max([thrust_ratio1, thrust_ratio2, thrust_ratio3]);

% Cruise
gammaCruise = 0;
thrust_ratio_cruise = 2*(0.5*rho_cruise*(V_cruise*kmph2mps)^2/(W_S_des*g)*((CD0_clean) + k_polare*(2*(W_S_des*g)*cos(gammaCruise)/(rho_cruise*(V_cruise*kmph2mps)^2))^2) + sin(gammaCruise));
...
thrust_ratio_ldg_climb = 2*(0.5*rho_SL*(vLdgClimb*kmph2mps)^2/(W_S_des*g)*((CD0_clean+C_D_flap+C_D_LG) + k_polare*(2*(W_S_des*g)*cos(gammaLdgClimb)/(rho_SL*(vLdgClimb*kmph2mps)^2))^2) + sin(gammaLdgClimb));

% Approach climb
vAppClimb = 1.41*Vstall;
gammaAppClimb = atan(2.1/100);
C_D_flap = 0.9 * (1/4.1935)^1.38 * Sflap/S_ref * sin(deg2rad(20))^2;
thrust_ratio_app_climb = 2*(0.5*rho_SL*(vAppClimb*kmph2mps)^2/(W_S_des*g)*((CD0_clean+C_D_flap) + k_polare*(2*(W_S_des*g)*cos(gammaAppClimb)/(rho_SL*(vAppClimb*kmph2mps)^2))^2) + sin(gammaAppClimb));
